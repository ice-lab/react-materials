...
export default function ColumnChart() {
  ...
  Shape.registerShape('interval', 'textInterval', {
    drawShape(cfg, group) {
      const self = this;
      const points = self.parsePoints(cfg.points); // 将0-1空间的坐标转换为画布坐标
      const { _origin: cfgOrign } = cfg.origin;

      const value = cfgOrign.value;
      group.addShape('text', {
        attrs: {
          text: value,
          textAlign: 'center',
  ...
  Shape.registerShape('interval', 'fallFlag', {
    ...
    drawShape(cfg, group) {
      ...
      if (cfgOrign === data[data.length - 1]) {
        return;
      }
      ...
      const p1 = points[0];
      const width = 9;
      const washaway = cfgOrign.washaway;
      group.addShape('text', {
        attrs: {
